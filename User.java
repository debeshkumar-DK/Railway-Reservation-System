...
public class User {
	...
	void inputreserve() throws Exception
	{
		...
		else		
		{
			...
			chart(pname,page,pgen,seat,coach,tnum,dt2);					
		}
	}
	...
	void tckt() 
	{
		...
		{
			System.out.println(e+"DK");
		}
	}
	
	void chart(String pname[],int page[],String pgen[],int seat,String coach,int tnum,java.sql.Date dt2)
	{
		try
		{
			String mysqlJDBCDriver = "com.mysql.cj.jdbc.Driver"; // jdbc driver
           ...
           Connection c = DriverManager.getConnection(url, user,
			...
			for(int i=0;i<seat;i++)
			{	
			ResultSet r2=s2.executeQuery("select * from train order by sno desc limit 1");
			r2.first();
			...
			PreparedStatement st=c.prepareStatement("insert into chart (pnr,name,age,gender,seatno,coach,status,timestamp,dot,tnum) values(?,?,?,?,?,?,?,?,?,?)");
				st.setLong(1,r1.getLong(1)+1);
				st.setString(2,pname[i]);
				st.setInt(3,page[i]);
				st.setString(4,pgen[i]);				
				if(r1.getInt(3)>0)			st.setInt(5,seat+1);
				else						st.setInt(5,0);
